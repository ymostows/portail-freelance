  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
            <h1 className="text-muted-foreground">Gérez et suivez vos missions freelances.</h1>
        </div>
        <CreateProjectDialog />
      </div>

      {/* Conteneur avec scroll pour les projets */}
      <div className="flex-1 overflow-y-auto min-h-0 pr-4">
        {/* 4. Gestion de l'état vide */}
        {(!projects || projects.length === 0) ? (
          <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed rounded-lg bg-muted/50 h-full">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 py-4">
            {projects.map((project) => (
              <Link key={project.id} href={`/dashboard/projects/${project.id}`} className="block h-full transition-transform hover:-translate-y-1">
                  <Card className="h-full">
                      ...
                      <CardFooter className="text-xs text-muted-foreground border-t bg-muted/20 mt-auto -mb-6 rounded-b-xl px-6 py-4">
